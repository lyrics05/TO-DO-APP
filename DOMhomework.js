...
var autor = document.getElementById("createdBy");
console.log(autor)
autor.innerHTML= `Aplicacion Creada por: Isidro Rivera`
...
function buildToDos(toDos) {
  // Tu código acá:
    var arr = toDos.map(function(ele,index){return buildToDo(ele,index)})
    return arr
}
...
function displayToDos() {
  ...
  var reco = buildToDos(toDoItems)
  for (let i  = 0; i < reco.length; i++) {
   toDoContainer.appendChild(reco[i])
